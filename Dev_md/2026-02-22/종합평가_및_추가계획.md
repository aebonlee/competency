...
| 결제 시스템 | ★★★★☆ | ★★★★☆ | 정상 동작 |
| **관리자 기능** | **★★★★☆** | **★★★☆☆** | **테이블명 불일치 12건** |
| **그룹 관리** | **★★★★☆** | **★★☆☆☆** | **테이블 미정의 3개 + 컬럼 불일치** |
| 통계/비교 | ★★★★☆ | ★★☆☆☆ | RPC 미구현, 컬럼명 불일치 |
| **전체** | **85%** | **70%** | **핵심 흐름은 정상, 관리/그룹 기능에 DB 불일치** |
...
| 3-4 | 에러 핸들링 강화 | Error Boundary 전역 적용 |

### Phase 4: 품질 개선 (중기)

| # | 작업 | 내용 |
|---|------|------|
| 4-1 | TypeScript 전환 | JSX → TSX (타입 안전성) |
| 4-2 | 코드 스플리팅 | React.lazy + Suspense (번들 최적화) |
| 4-3 | 테스트 작성 | Vitest 단위/통합 테스트 |
| 4-4 | SEO 최적화 | 메타 태그, OG 태그, sitemap |
| 4-5 | 접근성(a11y) 점검 | ARIA, 키보드 내비게이션 |
...
- **관리자/그룹 기능**은 코드는 완성되었으나 **DB 스키마와 불일치**하여 수정 필요
- Phase 1~2 (DB 정합성 수정)만 완료하면 **전체 완성도 90%+** 달성 가능
