...
const TOOLTIPS = {
  ...
  email: '이메일과 비밀번호로 로그인합니다. 회원가입 시 등록한 이메일을 사용하세요.'
};
      ...
      <div className="auth-center-wrapper">
        <div className="auth-card-google">
          <div className="auth-logo-area">
            ...
            <span className="brand-mcc-sub">4차산업혁명 핵심역량 검사</span>
          </div>
            ...
            <>
              <div className="auth-methods">
                <div className="auth-method-wrapper"
                  ...
                  <button className="auth-method-btn google" onClick={() => handleSocialLogin('google')} aria-label="Google 계정으로 로그인">
                  ...
                  {tooltip === 'google' && (
                    <div className="auth-tooltip">
                      <div className="auth-tooltip-arrow" />
                      {TOOLTIPS.google}
                    </div>
                  )}
                </div>
                <div className="auth-method-wrapper"
                  ...
                  onMouseLeave={() => setTooltip(null)}>
                  <button className="auth-method-btn kakao" onClick={() => handleSocialLogin('kakao')} aria-label="Kakao 계정으로 로그인">
                    <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
                ...
                <div className="auth-method-wrapper"
                  ...
                  onMouseLeave={() => setTooltip(null)}>
                  <button className="auth-method-btn email" onClick={() => setStep('email')} aria-label="이메일로 로그인">
            ...
            <>
              <form onSubmit={handleEmailLogin} className="auth-email-form">
                <div className="auth-input-group">
                  <input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="이메일 주소" aria-label="이메일 주소" required autoFocus />
                </div>
                <div className="auth-input-group">
                  <input type="password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} placeholder="비밀번호" aria-label="비밀번호" required />
                </div>
                {error && <div className="auth-error" role="alert">{error}</div>}
              ...
              <div style={{ textAlign: 'center', marginBottom: '12px' }}>
                <Link to="/forgot-password" style={{ fontSize: '13px', color: 'var(--text-light)' }}>비밀번호를 잊으셨나요?</Link>
              </div>
              <div className="auth-bottom-link">
                <span>계정이 없으신가요?</span>
                <Link to="/register">회원가입</Link>
